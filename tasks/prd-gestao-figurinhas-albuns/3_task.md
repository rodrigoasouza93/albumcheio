...
- [ ] 3.1 Criar `SupabaseModule` com clientes separados conforme a techspec.
- [ ] 3.2 Criar `AuthModule` com endpoints de cadastro, login e logout.
- [ ] 3.3 Criar `ProfilesModule` e endpoint `GET /api/v1/me`.
- [ ] 3.4 Implementar guard de autenticação para rotas protegidas.
- [ ] 3.5 Padronizar DTOs, validação de entrada e respostas de erro.
- [ ] 3.6 Mapear erros de autenticação para `400`, `401`, `403` e `422` conforme aplicável.
- [ ] 3.7 Garantir criação/consulta de perfil associada ao usuário Supabase.
- [ ] 3.8 Cobrir fluxos multiusuário básicos para preparar isolamento das coleções.
...
- [ ] Testes unitários: services de autenticação, criação/consulta de perfil, guard e tradução de erros Supabase.
- [ ] Testes de integração: endpoints `register`, `login`, `logout` e `me` com sucesso e falha.
- [ ] Testes E2E: cadastro/login básico pode ser coberto como preparação para a tarefa 9.
