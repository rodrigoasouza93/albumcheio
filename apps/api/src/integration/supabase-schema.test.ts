...
describe('supabase catalog and collection schema', () => {
  ...
  it('protects profile roles and restricts shared catalog mutations to admins', () => {
    const catalogPolicySnippets = [
      ...
      'create policy albums_select_authenticated on public.albums for select to authenticated using (public.can_read_catalog_album(albums.id, albums.status))',
      'create policy album_sections_select_authenticated on public.album_sections for select to authenticated using (public.can_read_catalog_album(album_sections.album_id))',
      'create policy stickers_select_authenticated on public.stickers for select to authenticated using (public.can_read_catalog_album(stickers.album_id))',
